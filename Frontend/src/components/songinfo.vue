<template>
  <div>
    <div class="song-details">
      ...
      <p>Genre: <router-link :to="'/genre/' + song.song_genre" class="btn btn-primary" v-if="song.song_genre">{{
        song.song_genre }}</router-link></p>
      <!-- {{ song.song_genre }} -->
      <p>
        Artists:
        <template v-for="(artist, index) in song.artists">
          <!-- <router-link :to="'/artist/' + artist.artist_id + '/' + user_id" class="btn btn-primary" :key="index">{{ artist.artist_name }}</router-link> -->
      ...
      <p>
        ...
        <template v-for="(album, index) in song.albums">
          <!-- <router-link :to="'/albums/' + album.id" class="btn btn-primary" :key="index">{{ album.name }}</router-link> -->
      ...
      <p v-if="rated">
        You rated this song: {{ ratevalue }}/5
      </p>
      <br>
      <!-- <button class="btn btn-primary" @click="play"><img src="" alt="Play"></button> -->
      <button class="btn btn-primary" @click="play">Play</button>
      <button class="btn btn-secondary" @click="showLyrics">Lyrics</button>
      <button class="btn btn-secondary" @click="showRating" style="background-color: purple;">Rate</button>
      <button class="btn btn-secondary" @click="addToPlaylist" style="background-color: rgb(224, 117, 35);">Add to
        playlist</button>
      <br><br>
      <audio :src="'http://127.0.0.1:5000/' + song.song_path" id="player" style="display: none; width: 600px;"controls></audio>
      <br>
      <form v-if="showAddToPlaylist" @submit.prevent="addToPlaylistSubmit">
        <label for="playlist_id">Select a Playlist:</label>
        <select v-model="selectedPlaylistId" required @change="handlePlaylistChange">
          <option value="" disabled>Select a Playlist</option>
          <option v-for="playlist in playlists" :value="playlist.playlist_id" :key="playlist.playlist_id">{{
        playlist.playlist_name }}</option>
          <option value="new">Create a New playlist</option>
        </select>
        <input type="text" v-model="newPlaylistName" v-if="creatingNewPlaylist" placeholder="Give Name for Playlist">
    ...
    <br><br>
    <p v-if="showlyrics" style="text-align: center; color: darkgreen; font-size: x-large; font-weight: 600;"> Lyrics
      <br><br> {{ song.song_lyrics }}
    </p>
    <br><br>
  </div>
</template>
  ...
  methods: {
    ...
    fetchSongData() {
      ...
      fetch(`http://127.0.0.1:5000/playlists/${parseInt(this.user_id)}/0`, {
        ...
        .then(response => response.json())
        .then(data => {
          this.playlists = data; // Assuming the response contains user's playlists
        })
        .catch(error => {
